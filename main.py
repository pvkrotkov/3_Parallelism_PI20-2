...
def read_matrix(filename):  # читаем матрицу из файла в список списков с типом элементов float
    try:
...
def write_matrix(filename, matrix):  # Записываем матрицу в файл
...
def write_element(filename, index, value, lock):  # Записываем отдельный элемент по индексу в файл матрицы
    i, j = index
    with lock:
...
def write_immediately(args):  # вычисление текущего элемента и его немедленная запись в файл
...
def create_arguments(filename, matrix1, matrix2):  # создание аргументов для pool'а процессов
    indexes = []
...
def generate_matrix(size, start, end):  # генерация случайной целочисленной матрицы
